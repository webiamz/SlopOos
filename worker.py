...
class TelegramWorker:
    ...
    def start(self) -> None:
        ...
        # Replit Keep-Alive background me chalu karo
    ...
    async def reconcile(self) -> None:
        ...
        for account in accounts:
            ...
            if not self.store.groups_for_account(account["id"]):
                ...
                if self.should_notify_account_error(account["id"]):
                    await self.notify_admins(
                        "SlotOps account needs assignment\n"
                        f"Account: {account['label']}\n"
                        "Reason: No group assigned.\n"
                        "Fix: /admin -> Assign Account"
                    )
                continue
    ...
    async def connect_account(self, account: dict[str, Any]) -> None:
        ...
        try:
            ...
            if not await client.is_user_authorized():
                raise RuntimeError("Session is not authorized. Run scripts/create_session.py again.")
            me = await client.get_me()

            assigned_groups = self.store.groups_for_account(account["id"])
            if not assigned_groups:
                raise RuntimeError("No group assigned. Use /assign ACCOUNT GROUP from admin bot.")
            ...
            for group in assigned_groups:
                try:
                    ...
                    self.store.log(
                        "warn",
                        "Skipped group handler because account cannot access group",
                        {
            ...
            self.store.log(
                "info",
                f'Connected account "{account["label"]}"',
                {"account_id": account["id"], "groups": [group["id"] for group in assigned_groups]},
            )
        except Exception as exc:
            error = str(exc)
            ...
            self.store.patch_account(
                account["id"],
                {
                    ...
                    "status": "error",
                    "last_error": error if not permanent else f"Disabled: {error}",
                },
            )
            self.store.log(
                "error",
                f'Failed to connect account "{account["label"]}"',
                {"account_id": account["id"], "error": error, "disabled": permanent},
            )
            if permanent:
                await self.notify_admins(
                    "SlotOps account disabled\n"
                    f"Account: {account['label']}\n"
                    f"Reason: {error}\n\n"
                    "Generate a real Telethon session with:\n"
                    "python scripts/create_session.py"
                )
            elif self.should_notify_account_error(account["id"]):
                await self.notify_admins(f'SlotOps account error:\n{account["label"]}: {error}')
        ...
        if not last_run or (not last_run.get("last_run_at") and not last_run.get("next_run_at")):
            ...
            if target_time_str:
                try:
                    ...
                    self.store.log("info", f"🎯 Target Time Set: {target_time_str} UTC", {"account": account_id})
                    return False
                except ValueError:
                    self.store.log("error", "Invalid time format. Falling back to jitter.", {})
            
            # 🎲 RANDOM JITTER BACKUP 🎲
            ...
            self.store.log("info", f"🎲 Random Jitter Set: Waiting {jitter_sec//3600}h {(jitter_sec%3600)//60}m", {"account": account_id})
            return False
    ...
    async def run_slot_cycle(
        ...
        # Reset memory in CustomDB for this cycle
        CustomDB.set(f"rem_{account['id']}", 999)
        
        self.store.log(
            "info",
            "Started scheduled slot cycle",
            {
                "account_id": account["id"],
                "group_id": group["id"],
                "command": command,
        ...
        try:
            ...
            # --- STAGE 2: 🌟 THE SMART FINISHER (RECOVERY LOOP) 🌟 ---
            await asyncio.sleep(6)
            
            rem_slots = CustomDB.get(f"rem_{account['id']}", 0)
            
            if 0 < rem_slots < 15: # Safety check
                self.store.log("info", f"Smart Finisher active: {rem_slots} slots missed by Zoro Bot. Recovering...", {"account_id": account["id"]})
            ...
            # --- STAGE 3: Auto Balance Check ---
            await asyncio.sleep(random.randint(3, 6))
            await client.send_message(peer, "/extols")

            # --- STAGE 4: ⏱️ THE ULTIMATE TIMER DECISION ⏱️ ---
            final_rem_slots = CustomDB.get(f"rem_{account['id']}", 999)
            
            if final_rem_slots == 0:
                # Perfect Sync: Zoro confirmed 0 slots. Lock it for 12 hours.
                ...
                self.store.log("info", "Slot count hit 0. Perfect 12-hour timer started.", {"account_id": account["id"]})
                
                await self.notify_admins(
                    "✅ Slot cycle completed perfectly\n"
                    f"Account: {account['label']}\n"
                    f"Group: {group['title']}\n"
                    f"Next run after: {settings['slot_interval_hours']} hours"
                )
            else:
                # Failsafe: Zoro bot was lagging too hard or offline. Check back in 1 hour.
                next_run = datetime.now(timezone.utc) + timedelta(hours=1)
                self.store.mark_scheduled_run(account["id"], group["id"], next_run.isoformat())
                self.store.log("warn", f"Zoro lag detected ({final_rem_slots} left). 1-hour Failsafe started.", {"account_id": account["id"]})
                
                await self.notify_admins(
                    "⚠️ Slot cycle Failsafe triggered\n"
                    f"Account: {account['label']}\n"
                    f"Remaining: {final_rem_slots} slots\n"
                    f"Retry scheduled in: 1 hour"
                )
            ...
            if "disconnected" in str(exc).lower():
                self.store.patch_account(account["id"], {"status": "offline", "last_error": str(exc)})
            self.store.log(
                "error",
                "Scheduled slot cycle failed",
    ...
    async def handle_message(
        ...
        # 🧠 SMART FINISHER EYE: Track Remaining Slots
        ...
        if any(emoji in text for emoji in SECRET_EMOJIS):
            ...
            if current_bal > 0:
                try:
                    ...
                    self.store.log("info", f"Secret Emoji Cashout: {current_bal} Extols", {"account_id": account["id"]})
                    
                    # Reset CustomDB balance to 0
                    CustomDB.set(f"bal_{account['id']}", 0)
                except Exception as e:
                    self.store.log("error", "Secret Cashout failed", {"error": str(e)})
            return

        # 💰 LIVE BALANCE & ALERT LOGIC
        if "current extols" in normalized:
            balance_match = re.search(r"extols[^\d]*([\d,]+)", text, re.IGNORECASE)
            if balance_match:
                current_balance = int(balance_match.group(1).replace(",", ""))
                
                # Save purely in CustomDB
                CustomDB.set(f"bal_{account['id']}", current_balance)
                
                self.store.log("info", f"Live Balance: {current_balance} Extols", {"account_id": account["id"]})
        ...
        if next_run_at:
            ...
            self.store.log(
                "info",
                "Detected group cooldown time",
                {
                    "account_id": account["id"],
                    "group_id": group["id"],
                    "message_id": message_id,
                    "next_run_at": selected_next_run.isoformat(),
            ...
            self.store.add_detection(
                {
                    ...
                    "action_status": "skipped_cooldown",
                }
            )
            return
        ...
        if settings["action"] == "log_only":
            ...
            self.store.log(
                "info",
                f'Slot keyword "{matched_keyword}" detected',
        ...
        try:
            ...
            self.store.log(
                "info",
                f'Sent configured response from "{account["label"]}"',
                {"account_id": account["id"], "group_id": group["id"]},
            )
        except Exception as exc:
            if "disconnected" in str(exc).lower():
            ...
            self.store.log("error", "Failed to send configured response", {"error": str(exc)})
    ...
    async def defer_for_flood_wait(
        ...
        self.store.log(
            "warn",
            "Telegram flood wait deferred slot work",
            {
                "account_id": account["id"],
                "group_id": group["id"],
                "source": source,
        ...
        if self.should_notify_account_error(account["id"]):
            await self.notify_admins(
                "Slot cycle paused by Telegram\n"
                f"Account: {account['label']}\n"
                f"Group: {group['title']}\n"
                f"Wait: {wait_seconds} seconds\n"
                f"Automatic retry after: {retry_at.isoformat()}"
            )
    ...
    async def ensure_client_connected(self, account: dict[str, Any], client: TelegramClient) -> None:
        ...
        self.store.log(
            "warn",
            "Telegram client disconnected; reconnecting before send",
            {"account_id": account["id"]},
        )
        await client.connect()
        if not await client.is_user_authorized():
    ...
    async def notify_admins(self, text: str) -> None:
        ...
        try:
            ...
            self.store.log("error", "Failed to notify Telegram admins", {"error": str(exc)})
