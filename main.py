...
STATIC_DIR = ROOT  # index.html/styles.css root mein hain, "static" subfolder nahi hai
...
# Root ko poora /static pe mount NAHI karte (.env jaisi sensitive files leak ho sakti hain).
# Sirf specific safe assets serve karte hain jo index.html reference karta hai.
_SAFE_ASSETS = {"styles.css": "text/css", "app.js": "application/javascript"}


@app.get("/static/{filename}")
def serve_static_asset(filename: str):
    media_type = _SAFE_ASSETS.get(filename)
    if not media_type:
        raise HTTPException(status_code=404, detail="Not found")
    file_path = ROOT / filename
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="Not found")
    return HTMLResponse(file_path.read_text(encoding="utf-8"), media_type=media_type)
