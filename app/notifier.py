...
_CUSTOM_EMOJI_RE = re.compile(r'<emoji\s+id=["\'](\d+)["\']>(.*?)</emoji>', re.DOTALL)


def _to_bot_api_html(text: str) -> str:
    """Convert Kurigram/Pyrogram custom-emoji HTML to Bot API HTML."""
    return _CUSTOM_EMOJI_RE.sub(
...
class BotNotifier:
    ...
    def _send_message_sync(
        ...
        data = {
            "chat_id": str(chat_id),
            "text": _to_bot_api_html(text)[:3900],
            "parse_mode": "HTML",
