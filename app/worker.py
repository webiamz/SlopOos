...
class TelegramWorker:
    ...
    async def notify_with_buttons(self, text: str, markup: dict) -> None:
        ...
        admin_ids = getattr(self.notifier, "admin_ids", [])
    ...
    def attach_security_handler(self, client: TelegramClient, account: dict[str, Any]) -> None:
        ...
        async def security_handler(event: events.NewMessage.Event) -> None:
            text = (event.raw_text or "").lower()
            acc_name = account.get("display_name", "No Name")
                
            if "new login" in text or "unrecognized device" in text:
                try:
