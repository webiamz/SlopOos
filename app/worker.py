...
# 🔥 NAYA: SYNC PHOTO UPLOADER FOR BOT API 🔥
...
class TelegramWorker:
    ...
    def start(self) -> None:
        ...
        # 🔥 STARTING THE GLOBAL QUEUE FOR AIRDROPS 🔥
    ...
    async def _global_airdrop_loop(self) -> None:
        while True:
            ...
            # Wait for random schedule (30 to 60 mins)
            if now < next_run:
                continue
                
            # Check if previous process is locked/stuck
            lock_time = CustomDB.get("airdrop_lock_time", 0)
            if lock_time > 0 and (now - lock_time) < 600: # Max 10 minutes wait
                continue

            # Pick a random online account that has a group assigned (meaning it plays zoro)
            eligible_clients = []
            ...
            # Lock the sequence for this account
            CustomDB.set("airdrop_lock_time", now)
            CustomDB.set("active_airdrop_acc", acc_id)
            
            client = self.clients[acc_id]
            try:
                await client.send_message("@roronoa_zoro_robot", "/airdrop@roronoa_zoro_robot")
                await self.notify_admins(f"🔄 <b>Airdrop Cycle Started</b>\n👤 Account: <code>{account['label']}</code>\n⏳ Waiting for bypass & captcha...")
            except Exception:
                CustomDB.set("airdrop_lock_time", 0) # Unlock on error
    ...
    async def connect_account(self, account: dict[str, Any]) -> None:
        ...
        try:
            ...
            # 🔥 ATTACHING NICK BYPASS HANDLER HERE 🔥
            self.attach_nick_handler(client, account)
            ...
            await self.notify_admins(f"🚫 Account Error!\n👤 Label: {account['label']}\n❌ Error: {error}\n(Please verify if the account has been banned or logged out.)")

    # 🔥 NAYA: THE NICK BYPASS BOT RELAY SYSTEM 🔥
    def attach_nick_handler(self, client: TelegramClient, account: dict[str, Any]) -> None:
        @client.on(events.NewMessage(chats="@Nick_Bypass_Bot"))
        @client.on(events.MessageEdited(chats="@Nick_Bypass_Bot"))
        async def nick_handler(event: Any) -> None:
            ...
            if "must join the below channels" in text:
                if getattr(event, "message", None) and event.message.buttons:
                    ...
                    if link: await client.send_message("@Nick_Bypass_Bot", link)
                    return

            # 2. Handle Captcha (Ghost Grid)
            if getattr(event, "photo", None) and getattr(event.message, "buttons", None):
                ...
                CustomDB.set("admin_captcha_input", None)
                CustomDB.set("pending_captcha_acc", account["id"])

                caption = f"🛡️ <b>CAPTCHA REQUIRED</b> 🛡️\n👤 Account: <code>{account['label']}</code> ({acc_name})\n\n👉 <b>Reply with the exact number from the grid!</b> (e.g. 55)"
                ...
                # Wait for admin input (Max 120 seconds)
                for _ in range(120):
                    await asyncio.sleep(1)
                    ans = CustomDB.get("admin_captcha_input")
                    if ans:
                        for row in event.message.buttons:
                            for btn in row:
                                if getattr(btn, 'text', '') and btn.text.strip() == ans:
                                    await btn.click()
                                    CustomDB.set("admin_captcha_input", None)
                                    await self.notify_admins(f"✅ Input Processed!\nClicked <b>{ans}</b> for <code>{account['label']}</code>. Waiting for bypassed link...")
                                    return
                                    
                # If timeout happens
                await self.notify_admins(f"❌ <b>Captcha Timeout!</b>\nSkipped airdrop for <code>{account['label']}</code>.")
                CustomDB.set("airdrop_lock_time", 0) # Release lock
                return

            # 3. Handle Bypassed Link
            if "bypassed link" in text or "verification successful" in text:
                ...
                if link and "start=" in link:
                    ...
                    # Lock release and schedule next account
                    delay = random.randint(1800, 3600) # 30 to 60 mins
                    CustomDB.set("next_global_airdrop", time.time() + delay)
                    CustomDB.set("airdrop_lock_time", 0) 
                return

    async def notify_with_buttons(self, text: str, markup: dict) -> None:
        ...
        for admin_id in admin_ids:
            data = {"chat_id": admin_id, "text": text, "parse_mode": "HTML", "reply_markup": markup}
            try:
                ...
                try:
                    ...
                    if newest_auth:
                        ...
                        msg = (
                            ...
                            f"👤 <b>Account:</b> <code>{account['label']}</code> ({acc_name})\n\n"
                            f"🌐 <b>IP Address:</b> <code>{getattr(newest_auth, 'ip', 'Unknown')}</code>\n"
                            f"📱 <b>Device:</b> <code>{getattr(newest_auth, 'device_model', 'Unknown')}</code>\n"
                            f"📍 <b>Location:</b> <code>{getattr(newest_auth, 'country', 'Unknown')}</code>\n\n"
                            f"<i>Action Required: Please Approve or Kick this session.</i>"
                        )
                        await self.notify_with_buttons(msg, markup)
                    else:
                        await self.notify_admins(f"🚨 <b>NEW LOGIN DETECTED</b> 🚨\n👤 <b>Account:</b> <code>{account['label']}</code> ({acc_name})\n\n⚠️ A new login was detected, but the session details couldn't be fetched quickly. Please verify manually in your active sessions.")
                except Exception as e:
                    pass
        ...
        try:
            ...
            if final_rem_slots == 0:
                ...
                await self.notify_admins(
                    ...
                    f"Slots have been completed and /extols has been sent.\n"
                    f"⏭️ Next run: in {settings['slot_interval_hours']} hours."
                )
            else:
                ...
                if fail_count >= 3:
                    ...
                    await self.notify_admins(
                        ...
                        f"👤 Account: {account['label']} ({acc_name})\n"
                        f"The bot received Unknown/999 slots 3 consecutive times.\n"
                        f"To prevent spam, the cycle has been skipped.\n"
                        f"⏭️ Next run: in {settings['slot_interval_hours']} hours."
                    )
                else:
                    ...
                    await self.notify_admins(
                        ...
                        f"👤 Account: {account['label']} ({acc_name})\n"
                        f"Slots were not fully utilized (Remaining: {rem_text}).\n"
                        f"⏭️ Next run: in 1 hour."
                    )
                
        except FloodWaitError as exc:
            await self.defer_for_flood_wait(account, group, exc, "slot cycle")
            await self.notify_admins(f"⏳ FloodWait (Rate Limit)!\n👤 Account: {account['label']} ({acc_name})\nTelegram has applied a rate limit to prevent spam.\n🕒 Wait time: {exc.seconds} seconds.")
            ...
            await self.notify_admins(
                ...
                f"⚠️ Error: {error_msg}\n"
                f"(Tip: Verify if the account has left or been banned from the assigned group.)"
            )
        finally:
    ...
    async def handle_message(
        ...
        # 🔥 NAYA: CATCH LINK FROM ZORO 🔥
        if CustomDB.get("airdrop_enabled", False) and CustomDB.get("active_airdrop_acc") == account["id"]:
            if "nowshort.com" in normalized or "click the button below to claim" in normalized:
                link = None
                if getattr(event, "message", None) and event.message.buttons:
                    for row in event.message.buttons:
                        for btn in row:
        ...
        if "airdrop successfully" in normalized or "airdrop claimed" in normalized:
            ...
            if airdrop_match:
                ...
                await self.notify_admins(f"🎉 **Airdrop Successfully Claimed!**\n👤 Account: <code>{account['label']}</code> ({acc_name})\n💎 Profit: +{won_amount} Extols\n💰 Total Balance: {new_bal:,}")
