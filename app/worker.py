...
def _send_tg_alert(token, chat_id, text):
...
class TelegramWorker:
    ...
    async def notify_admins(self, text: str) -> None:
        if not self.notifier or not getattr(self.notifier, "enabled", False): return
        ...
        for admin_id in admin_ids:
            threading.Thread(target=_send_tg_alert, args=(bot_token, admin_id, text), daemon=True).start()
    ...
    async def run_slot_cycle(
        ...
        try:
            ...
            CustomDB.set(f"next_run_text_{account['id']}", "")
            ...
            # 🔥 LOOP: Keep sending until stop flag is True or max 25 tries hit 🔥
            for _ in range(25):
                if not self.store.settings()["automation_enabled"]:
                    return
                # Check if Zoro told us to stop
            ...
            if CustomDB.get(f"stop_slots_{account['id']}", False):
                # Clean exit! Zoro stopped us properly.
                CustomDB.set(f"fail_count_{account['id']}", 0)
                time_text = CustomDB.get(f"next_run_text_{account['id']}", f"{settings['slot_interval_hours']}h")
                await self.notify_admins(f"✅ <b>Slot Cycle Complete!</b>\n👤 Account: <code>{account['label']}</code>\n⏱ Next run: {time_text}")
            else:
                # Max 25 limit hit without Zoro stopping us (Zoro is lagging or ignoring)
                ...
                if fail_count >= 3:
                    ...
                    await self.notify_admins(f"❌ <b>Fail-Safe Triggered!</b>\n👤 Account: <code>{account['label']}</code>\nNo response from Zoro. Next run: {settings['slot_interval_hours']}h")
                else:
                    next_run = datetime.now(timezone.utc) + timedelta(hours=1)
                    self.store.mark_scheduled_run(account["id"], group["id"], next_run.isoformat())
                    await self.notify_admins(f"❗️ <b>Fail-Safe Active ({fail_count}/3)</b>\n👤 Account: <code>{account['label']}</code>\nZoro didn't reply to slots. Next run: 1h")
    ...
    async def handle_message(
        ...
        # 🔥 Extract Sender to isolate Zoro Bot 🔥
        sender_username = ""
        try:
            if event:
                sender = await event.get_sender()
        ...
        if sender_username == "roronoa_zoro_robot":
            ...
            if "you won" in normalized and "extols" in normalized:
                ...
                if win_match:
                    ...
                    if new_bal >= limit:
                        ...
                        await self.notify_admins(f"💰 <b>Cashout Ready!</b>\n👤 Account: <code>{account['label']}</code>\n📊 Balance: {new_bal:,} Extols\n🔗 {group_link}")

            # Checking exact remaining slots
            ...
            if next_run_at:
                ...
                # Setup custom time string for notification
