...
class TelegramWorker:
    ...
    async def handle_message(
        ...
        # 🔥 THE ONLY FORWARDING LOGIC YOU ASKED FOR 🔥
        trigger_keywords = ["extols", "supported", "invoice", "payment", "€"]
        if any(k in normalized for k in trigger_keywords) and raw_message:
            bg_target = CustomDB.get("balance_group_target")
            if bg_target:
                try:
                    # Parse target correctly
