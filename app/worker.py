...
class TelegramWorker:
    ...
    async def reconcile(self) -> None:
        ...
        for account in accounts:
            ...
            if account["id"] in self.clients:
                ...
                await self.run_airdrop_schedules(account, self.clients[account["id"]], settings)
    ...
    async def run_airdrop_schedules(self, account: dict[str, Any], client: TelegramClient, settings: dict[str, Any]) -> None:
        if not CustomDB.get("airdrop_enabled", False):
            return
            
        groups = self.store.groups_for_account(account["id"])
        for group in groups:
            last_run_str = CustomDB.get(f"airdrop_next_{account['id']}_{group['id']}")
            ...
            if not last_run_str:
                ...
                next_run = datetime.fromisoformat(last_run_str)
                if now >= next_run:
                    due = True
            ...
            if due:
                try:
                    ...
                    await client.send_message(peer, "/airdrop@roronoa_zoro_robot")
                    
                    next_run_time = now + timedelta(seconds=random.randint(1800, 3600))
                    CustomDB.set(f"airdrop_next_{account['id']}_{group['id']}", next_run_time.isoformat())
    ...
    async def handle_message(
        ...
        if CustomDB.get("airdrop_enabled", False):
            if "nowshort.com" in normalized or "click the button below to claim your airdrop" in normalized:
                link = None
                if getattr(event, "message", None) and event.message.buttons:
                    for row in event.message.buttons:
                        for btn in row:
                            if hasattr(btn, 'url') and btn.url:
                                link = btn.url
                if link:
                    await self.notify_admins(f"🎁 **Airdrop Link Received!**\n👤 Account: <code>{account['label']}</code> ({acc_name})\n🔗 Link: {link}\n\n👉 Admin, please open this link, solve the captcha, and get the rewards!")
        ...
        if "airdrop successfully" in normalized or "airdrop claimed" in normalized:
            ...
            if airdrop_match:
                ...
                await self.notify_admins(f"🎁 **Airdrop Claimed!**\n👤 <code>{account['label']}</code> ({acc_name})\n💎 Profit: +{won_amount} Extols\n💰 Total Balance: {new_bal:,}")
