...
class TelegramWorker:
    ...
    async def _global_airdrop_loop(self) -> None:
        while True:
            ...
            try:
                # 🔥 Send explicitly in Zoro's Private DM for cleaner workflow
    ...
    async def connect_account(self, account: dict[str, Any]) -> None:
        ...
        try:
            ...
            # Private Handlers manage BOTH Nick Bypass AND Zoro Airdrops
    ...
    def attach_private_handlers(self, client: TelegramClient, account: dict[str, Any]) -> None:
        ...
        async def private_handler(event: Any) -> None:
                            ...
                            for row in event.message.buttons:
                                for btn in row:
                                    if getattr(btn, 'text', '') and btn.text.strip() == ans:
                                        await btn.click()
                                        CustomDB.set("admin_captcha_input", None)
                                        await self.notify_admins(f"✅ <b>Input Processed!</b>\nClicked <b>{ans}</b> for <code>{account['label']}</code>.\n⏳ Waiting for bypassed link...")
                                        return
            ...
            # ==========================================
            # 2. ZORO BOT PRIVATE DM LOGIC (The Fix)
            # ==========================================
            if sender_username == "roronoa_zoro_robot":
                
                # CATCHING THE AD LINK
                ...
                if "airdrop successfully" in text or "airdrop claimed" in text:
                    ...
                    if airdrop_match:
                        ...
                        await self.notify_admins(f"🎉 <b>Airdrop Successfully Claimed!</b>\n👤 Account: <code>{account['label']}</code> ({acc_name})\n💎 Profit: +{won_amount} Extols\n💰 Total Balance: {new_bal:,}")
        ...
        try:
            ...
            if final_rem_slots == 0:
                ...
                if fail_count >= 3:
                    ...
                    await self.notify_admins(
                        f"⚠️ <b>Fail-Safe Triggered! ({fail_count}/3)</b>\n"
                        f"👤 Account: <code>{account['label']}</code>\n"
            ...
            await self.notify_admins(
                f"❌ <b>Action Failed (Group/Message Error)!</b>\n"
                f"👤 Account: <code>{account['label']}</code>\n"
        ...
        if "you won" in normalized and "extols" in normalized:
            ...
            if win_match:
                ...
                if new_bal >= limit:
                    ...
                    await self.notify_admins(
                        f"🚨 <b>Cashout Ready!</b> 🚨\n"
                        f"👤 Account: <code>{account['label']}</code> ({acc_name})\n"
