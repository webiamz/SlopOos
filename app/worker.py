...
class TelegramWorker:
    ...
    async def run_slot_cycle(
        ...
        try:
            ...
            # 🔥 SMART LOOP: Fires max 25 times, stops instantly if Zoro says 0 or limits out
            for _ in range(25):
            ...
            if exact_time:
                # Zoro gave us exact time (e.g. 11h 58m) -> Use it and overwrite fail-safes
                self.store.mark_scheduled_run(account["id"], group["id"], exact_time)
                CustomDB.set(f"exact_next_run_{account['id']}_{group['id']}", None)
                time_text = CustomDB.get(f"next_run_text_{account['id']}", "")
                await self.notify_admins(f"{E_CHK} <b>Slot Scheduled!</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_TIME} Exact Time Read. Next run in {time_text}.")
                
            elif final_rem_slots == 0 or CustomDB.get(f"stop_slots_{account['id']}", False):
                # Clean exit but no exact time provided by Zoro
                ...
                # Max 25 limit hit without Zoro stopping us (Zoro is lagging)
                ...
                if fail_count >= 3:
                    ...
                    next_run = datetime.now(timezone.utc) + timedelta(hours=int(settings["slot_interval_hours"]))
                    self.store.mark_scheduled_run(account["id"], group["id"], next_run.isoformat())
                    await self.notify_admins(f"{E_ERR} <b>Fail-Safe Triggered!</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_WARN} Zoro offline. Next run: {settings['slot_interval_hours']}h")
                else:
    ...
    async def handle_message(
        ...
        # 🔥 MAIN ACCOUNT EXCLUSION FROM CASHOUT 🔥
        SECRET_EMOJIS = ["🗿", "🤧", "🌚", "🥲"]
        if any(emoji in text for emoji in SECRET_EMOJIS):
            # ARMAN ID CHECK (Returns immediately if it's the main account)
            if str(account['id']) == str(ARMAN_ID):
                return
        ...
        if sender_username == "roronoa_zoro_robot":
            ...
            # 🔥 SMART TIME READER (Saves Exact Time to DB) 🔥
