...
E_SYNC = ce("🔄", "5260687119092817530")
E_WARN = ce("❗️", "5258474669769497337")
E_GIFT = ce("📦", "5258134813302332906")
...
class TelegramWorker:
    ...
    async def _global_airdrop_loop(self) -> None:
        while True:
            ...
            try:
                ...
                await self.notify_admins(f"{E_SYNC} <b>Airdrop Triggered</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_TIME} Waiting for link...")
            except Exception:
    ...
    def attach_private_handlers(self, client: TelegramClient, account: dict[str, Any]) -> None:
        ...
        async def private_handler(event: Any) -> None:
            ...
            # 🔥 THE CRASH FIX: Safe Username Extraction 🔥
            raw_username = getattr(sender, 'username', None) if sender else None
            sender_username = str(raw_username).lower() if raw_username else ""
            ...
            if sender_id == ARMAN_ID:
                if text.strip().isdigit() and len(text.strip()) <= 3:
                    ...
                    await event.reply(f"{E_CHK} Got it: {text.strip()}! Pressing button...")
                    return
            ...
            if sender_id == 8226002644 or sender_username == "nick_bypass_bot":
                ...
                if getattr(event, "photo", None) and getattr(event.message, "buttons", None):
                    ...
                    await self.notify_admins(f"{E_SEC} <b>Captcha Required</b>\n{E_USER} Account: <code>{account['label']}</code>\n👉 <i>Sent to Arman's DM. Reply there!</i>")

                    try:
                        await client.send_file(
                            ARMAN_ID, 
                            photo_path, 
                            caption=f"{E_WARN} CAPTCHA ALERT\nAccount: {account['label']}\n👉 Reply to this message with the exact number!"
                        )
                    except Exception:
                        pass
                            ...
                            for row in event.message.buttons:
                                for btn in row:
                                    if getattr(btn, 'text', '') and btn.text.strip() == ans:
                                        await btn.click()
                                        CustomDB.set(f"captcha_ans_{account['id']}", None)
                                        await self.notify_admins(f"{E_CHK} <b>Captcha Solved</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_TIME} Waiting for bypass...")
                                        return
                    
                    try:
                        await client.send_message(ARMAN_ID, f"{E_ERR} Timeout! Captcha skipped for {account['label']}.")
                        await self.notify_admins(f"{E_ERR} <b>Captcha Timeout!</b>\n{E_USER} Account: <code>{account['label']}</code>")
                    except: pass
                    CustomDB.set("airdrop_lock_time", 0) 
                    return
                ...
                if "bypassed link" in text or "verification successful" in text:
                    ...
                    if link and "start=" in link:
                        ...
                        await self.notify_admins(f"{E_CHK} <b>Bypass Success</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_TIME} Claiming payload...")
            ...
            if sender_username == "roronoa_zoro_robot":
                if CustomDB.get("airdrop_enabled", False) and CustomDB.get("active_airdrop_acc") == account["id"]:
                    if "nowshort.com" in text or "click the button below to claim" in text:
                        ...
                        if link:
                            ...
                            await self.notify_admins(f"{E_LINK} <b>Link Caught</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_TIME} Forwarding to Nick Bypass...")
                ...
                if "airdrop successfully" in text or "airdrop claimed" in text:
                    ...
                    if airdrop_match:
                        ...
                        await self.notify_admins(f"{E_GIFT} <b>Airdrop Claimed!</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_STATS} Profit: +{won_amount} Extols\n{E_MONEY} Total: {new_bal:,}")
        ...
        try:
            ...
            if final_rem_slots == 0:
                ...
                if fail_count >= 3:
                    ...
                    next_run = datetime.now(timezone.utc) + timedelta(hours=int(settings["slot_interval_hours"]))
                    self.store.mark_scheduled_run(account["id"], group["id"], next_run.isoformat())
                    await self.notify_admins(f"{E_ERR} <b>Fail-Safe Triggered (Skipped)!</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_TIME} Next run: {settings['slot_interval_hours']}h")
                else:
        ...
        if "you won" in normalized and "extols" in normalized:
            ...
            if win_match:
                ...
                if new_bal >= limit:
                    ...
                    await self.notify_admins(f"{E_MONEY} <b>Cashout Ready!</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_STATS} Balance: {new_bal:,} Extols\n{E_LINK} {group_link}")
        ...
        if "your current extols:" in normalized or "not supported" in normalized:
            if "your current extols:" in normalized:
                ...
                if bal_match:
                    ...
                    if exact_bal >= limit:
                        group_link = group.get("identifier", "Unknown")
                        await self.notify_admins(f"{E_MONEY} <b>Cashout Ready (Synced)!</b>\n{E_USER} Account: <code>{account['label']}</code>\n{E_STATS} Balance: {exact_bal:,}\n{E_LINK} {group_link}")
