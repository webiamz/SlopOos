...
class TelegramWorker:
    ...
    async def run_slot_cycle(
        ...
        try:
            ...
            # 🔥 SENDS /EXTOLS & TURNS FLAG ON 🔥
            CustomDB.set(f"extols_flag_{account['id']}", True) # FLAG ON
    ...
    def attach_handler(
        ...
        async def handler(event: events.NewMessage.Event) -> None:
            ...
            if not text and event.message:
                try:
                    text = str(event.message.stringify())
                except Exception:
                    pass

            # Pushing raw message to handle_message
    ...
    async def handle_message(
        ...
        # 🔥 SMART FLAG FORWARDING LOGIC 🔥
        # Agar flag ON hai, iska matlab ye message theek /extols ke baad aaya hai
        if CustomDB.get(f"extols_flag_{account['id']}", False) and raw_message:
            # FLAG turant OFF kar do taaki agla koi bhi faltu message (slot rewards) forward na ho
            CustomDB.set(f"extols_flag_{account['id']}", False) 
            ...
            if bg_target:
                try:
                    ...
                    # Forward the exact raw message and label it
                    fwd = await client.forward_messages(target_entity, raw_message)
                    if fwd:
                        await client.send_message(target_entity, f"👤 Account: `{account['label']}`", reply_to=fwd.id)
