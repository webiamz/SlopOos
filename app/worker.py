...
class TelegramWorker:
    ...
    async def _global_airdrop_loop(self) -> None:
        while True:
            ...
            try:
                ...
                await self.notify_admins(f"🔄 Airdrop Started | Account: {account['label']}\n⏳ Waiting for bypass...")
            except Exception:
    ...
    async def connect_account(self, account: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.notify_admins(f"🚫 Account Error! | Label: {account['label']} | Error: {error}")
    ...
    def attach_private_handlers(self, client: TelegramClient, account: dict[str, Any]) -> None:
        ...
        async def private_handler(event: Any) -> None:
            ...
            if sender_id == 8226002644 or sender_username == "nick_bypass_bot":
                ...
                # 🔥 CATCHING PHOTO AND SENDING TO ARMAN'S DM 🔥
                if getattr(event, "photo", None) and getattr(event.message, "buttons", None):
                    ...
                    try:
                        await client.send_file(
                            ARMAN_ID, 
                            event.message.media, 
                            caption=f"🚨 CAPTCHA ALERT 🚨\nAccount: {account['label']}\n👉 Reply to this message with the exact number from the grid!"
                        )
                    except Exception as e:
                        await self.notify_admins(f"⚠️ Could not DM Arman! Please send a message to {account['label']} first to allow DMs. Error: {e}")

                    # Waiting loop for Arman's answer
                    for _ in range(120):
            ...
            if sender_username == "roronoa_zoro_robot":
                ...
                if CustomDB.get("airdrop_enabled", False) and CustomDB.get("active_airdrop_acc") == account["id"]:
                    if "nowshort.com" in text or "click the button below to claim" in text:
                        ...
                        if link:
                            ...
                            # Start nick bypass first, then send link
                ...
                if "airdrop successfully" in text or "airdrop claimed" in text:
                    ...
                    if airdrop_match:
                        ...
                        await self.notify_admins(f"🎉 Airdrop Claimed! | Account: {account['label']} | Profit: +{won_amount} Extols | Total: {new_bal:,}")
                ...
                try:
                    ...
                    if newest_auth:
                        ...
                        msg = (
                            ...
                            f"IP: {getattr(newest_auth, 'ip', 'Unknown')}\n"
                            f"Device: {getattr(newest_auth, 'device_model', 'Unknown')}\n"
                            f"Location: {getattr(newest_auth, 'country', 'Unknown')}\n"
                            f"Please Approve or Kick from Admin Panel."
                        )
                        await self.notify_with_buttons(msg, markup)
                    else:
                        await self.notify_admins(f"🚨 NEW LOGIN DETECTED 🚨\nAccount: {account['label']}\nPlease verify manually.")
                except Exception as e:
                    pass
    ...
    async def run_slot_cycle(
        ...
        try:
            ...
            if final_rem_slots == 0:
                ...
                await self.notify_admins(f"✅ Slot Cycle Complete! | Account: {account['label']}\n⏭️ Next run: in {settings['slot_interval_hours']} hours.")
            else:
                ...
                if fail_count >= 3:
                    ...
                    next_run = datetime.now(timezone.utc) + timedelta(hours=int(settings["slot_interval_hours"]))
                    self.store.mark_scheduled_run(account["id"], group["id"], next_run.isoformat())
                    await self.notify_admins(f"🚫 Fail-Safe Triggered (Skipped)! | Account: {account['label']}\n⏭️ Next run: in {settings['slot_interval_hours']} hours.")
                else:
                    next_run = datetime.now(timezone.utc) + timedelta(hours=1)
                    self.store.mark_scheduled_run(account["id"], group["id"], next_run.isoformat())
                    rem_text = final_rem_slots if final_rem_slots != 999 else "Unknown"
                    await self.notify_admins(f"⚠️ Fail-Safe Active! ({fail_count}/3) | Account: {account['label']} (Rem: {rem_text})\n⏭️ Next run: in 1 hour.")
                
        except FloodWaitError as exc:
            await self.defer_for_flood_wait(account, group, exc, "slot cycle")
            await self.notify_admins(f"⏳ FloodWait Limit! | Account: {account['label']}\n🕒 Wait time: {exc.seconds} seconds.")
            ...
            await self.notify_admins(f"❌ Action Failed! | Account: {account['label']}\n⚠️ Error: {error_msg}")
        finally:
        ...
        if "you won" in normalized and "extols" in normalized:
            ...
            if win_match:
                ...
                if new_bal >= limit:
                    ...
                    await self.notify_admins(f"🚨 Cashout Ready! | Account: {account['label']} | Live Balance: {new_bal:,}\n🔗 {group_link}")
        ...
        if "your current extols:" in normalized or "not supported" in normalized:
            ...
            if "your current extols:" in normalized:
                ...
                if bal_match:
                    ...
                    if exact_bal >= limit:
                        group_link = group.get("identifier", "Unknown")
                        await self.notify_admins(f"🚨 Cashout Ready (Synced)! | Account: {account['label']} | Balance: {exact_bal:,}\n🔗 {group_link}")
