...
class TelegramWorker:
    ...
    async def run_slot_cycle(
        ...
        try:
            ...
            # 🔥 ADMIN NOTIFICATION FOR MANUAL BALANCE UPDATE 🔥
            short_id = account["id"][:6]
            await self.notify_admins(
                f"🔔 **MANUAL CHECK REQUIRED** 🔔\n"
                f"👤 Account: `{account['label']}`\n"
                f"Bot ne 12 slots pure karke `/extols` bhej diya hai.\n\n"
                f"👉 Asli app me Zoro ka reply dekho aur ye command bhejo:\n"
                f"`/set_bal {short_id} BALANCE`"
            )
    ...
    async def handle_message(
        ...
        # 🔥 YEH PURANA BALANCE CODE WAISE HI RAKHA HAI 🔥
        if "extols" in normalized:
