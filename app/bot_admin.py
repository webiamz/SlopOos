...
/bal
/mass_refer (Start Referral Setup)
/stop_ref (Stop ongoing Referral)
"""
    ...
    async def _poll_loop(self) -> None:
        ...
        while True:
            try:
                ...
                # Removed constant spam error logging to avoid unwanted notifications
        ...
        try:
            ...
            # --- DELETE ALL ACCOUNTS ---
                ...
                for acc in list(self.store.accounts()):
                    ...
                    count += 1
                await self.reply(chat_id, f"✅ All {count} accounts deleted successfully.", accounts_keyboard(), message_id=message_id)
            ...
            # --- DELETE ALL GROUPS ---
                ...
                for grp in list(self.store.groups()):
                    ...
                    count += 1
                await self.reply(chat_id, f"✅ All {count} groups deleted successfully.", groups_keyboard(), message_id=message_id)
            ...
            # --- ASSIGNMENTS ---
                ...
                group_id = group["id"]
                await self.assign_ids(chat_id, account_id, group_id, message_id=message_id)
            ...
            # --- OTHERS ---
                ...
                await self.reply(chat_id, "Add Admin\n\nSend:\n/add_admin USER_ID\n\nExample:\n/add_admin 123456789", admins_keyboard(), message_id=message_id)
            elif data == "delete_admin_help":
                await self.reply(chat_id, "Remove Admin\n\nSend:\n/del_admin USER_ID\n\nEnv owner admins cannot be removed from panel.", admins_keyboard(), message_id=message_id)
                ...
                await self.reply(chat_id, "Set Cashout Limit\n\nSend:\n/limit AMOUNT\n\nExample:\n/limit 1000", settings_keyboard(), message_id=message_id)
            elif data == "action_log":
                self.store.update_settings({"action": "log_only"})
                await self.reply(chat_id, "Action set: log only.", settings_keyboard(), message_id=message_id)
                ...
                await self.reply(chat_id, "To set cycle, send:\n/set_cycle 12", settings_keyboard(), message_id=message_id)
            elif data == "keywords_help":
                await self.reply(chat_id, "To set keywords, send:\n/set_keywords slot,available,booking", settings_keyboard(), message_id=message_id)
            elif data == "help":
                await self.reply(chat_id, self.get_help_text(), main_keyboard(), message_id=message_id)
            elif data == "test_alert":
                await self.reply(chat_id, "Test alert ok.", main_keyboard(), message_id=message_id)
            else:
                await self.reply(chat_id, "Unknown button. Tap /admin.", main_keyboard(), message_id=message_id)
        except Exception as exc:
    ...
    async def stop_refer(self, chat_id: int) -> None:
        if chat_id in self.active_tasks:
            ...
            await self.notifier.send_message(chat_id, "🛑 Mass referral task has been forcibly STOPPED!")
        else:
            await self.notifier.send_message(chat_id, "No active referral task found.")

    async def execute_mass_refer(self, chat_id: int, args: str) -> None:
        try:
            parts = [p.strip() for p in args.split("|")]
            if len(parts) != 3:
                await self.notifier.send_message(chat_id, "❌ Usage: /ref all | @TargetBot | /start ref123")
                return
            ...
            if not channels_to_join:
                await self.notifier.send_message(chat_id, "❌ No channels set! Run /mass_refer first.")
                return
            ...
            if acc_target.lower() == "all":
                ...
                labels = [l.strip().lower() for l in acc_target.split(",")]
                target_accounts = [acc for acc in all_accounts if acc["label"].lower() in labels]
                
            if not target_accounts:
                await self.notifier.send_message(chat_id, "❌ No matching accounts found.")
                return
                
            await self.notifier.send_message(
                chat_id, 
                f"🚀 Initiating Referrals...\nAccounts: {len(target_accounts)}\nChannels: {len(channels_to_join)}\n\n(Waiting 1-2 mins between accounts for safety. Use /stop_ref to cancel anytime.)"
            )
            
            success_count = 0
            for i, acc in enumerate(target_accounts):
                # Check for cancellation before proceeding to next account
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        ...
                        # 2. Send refer message
                        await client.send_message(target_bot, refer_msg)
                        await asyncio.sleep(4)
                        
                        # 3. Read messages and click button (Claim)
                        ...
                        if messages and messages[0].buttons:
                            ...
                            if not clicked:
                                try:
                                    last_row = len(msg.buttons) - 1
                                    last_col = len(msg.buttons[last_row]) - 1
                                    await msg.click(last_row, last_col)
                                    await asyncio.sleep(2)
                                except Exception:
                                    pass

                        # 4. Fetch the final reply from the bot & forward to Admin
                        ...
                        await self.notifier.send_message(chat_id, f"📢 Bot Reply for {acc['label']}:\n{bot_reply_text}")
                        
                        # 5. Success confirmation
                        success_count += 1
                        await self.notifier.send_message(chat_id, f"✅ Account {i+1} complete: {acc['label']}")
                    ...
                    await self.notifier.send_message(chat_id, f"❌ Account {i+1} failed: {acc['label']}")
                    
            if chat_id in self.active_tasks:
                del self.active_tasks[chat_id]
            await self.notifier.send_message(chat_id, f"🎉 Mass referral task finished!\nSuccessfully executed on {success_count}/{len(target_accounts)} accounts.")
            
        except asyncio.CancelledError:
            pass # Stopped intentionally
        except Exception as e:
            await self.notifier.send_message(chat_id, f"Error: {str(e)}")
    ...
    async def handle_command(self, chat_id: int, text: str) -> None:
        ...
        try:
            # --- INTERACTIVE STATE CHECKER FOR /mass_refer ---
                ...
                if state["step"] == "waiting_count":
                    ...
                    if current < total:
                        ...
                        await self.reply(
                            ...
                            "`/ref all | @TargetBot | /start ref123`\n\n"
                            "*Note: Replace 'all' with exact account labels (comma-separated) to use specific accounts.*"
                        )
                    return
            ...
            if command in {"/start", "/admin"}:
                ...
                await self.reply(chat_id, "How many channels or groups do you need to join?")
            elif command == "/cancel":
                if chat_id in REFER_STATE:
                    del REFER_STATE[chat_id]
    ...
    async def show_account_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        if not accounts:
            await self.reply(chat_id, "No accounts yet. Tap Add Account first.", accounts_keyboard(), message_id=message_id)
            return
    ...
    async def show_delete_account_picker(self, chat_id: int, message_id: int | None = None) -> None:
        accounts = self.store.accounts()
        if not accounts:
            await self.reply(chat_id, "No accounts to delete.", accounts_keyboard(), message_id=message_id)
            return
        rows = [
    ...
    async def show_delete_group_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        if not groups:
            await self.reply(chat_id, "No groups to delete.", groups_keyboard(), message_id=message_id)
            return
        rows = [
    ...
    async def show_group_picker(self, chat_id: int, account_id: str, message_id: int | None = None) -> None:
        ...
        groups = self.store.groups()
        if not account:
            await self.reply(chat_id, "Account not found.", main_keyboard(), message_id=message_id)
            return
        if not groups:
            await self.reply(chat_id, "No groups yet. Tap Add Group first.", groups_keyboard(), message_id=message_id)
            return
    ...
    async def reply(self, chat_id: int, text: str, markup: dict | None = None, message_id: int | None = None) -> None:
        try:
            if message_id:
                await asyncio.to_thread(self._edit_message_sync, chat_id, message_id, text, markup)
            else:
    ...
    def _edit_message_sync(self, chat_id: int, message_id: int, text: str, markup: dict | None = None) -> None:
        ...
        try:
            ...
            # If a user double taps a button rapidly, Telegram says "message is not modified".
            # Hum is error ko ignore kar denge taaki bot crash na ho.
            if "message is not modified" not in error_body.lower():
