...
class CustomDB:
    ...
    def _start_sync(cls):
        if not cls._sync_started:
            ...
            def syncer():
                while True:
                    time.sleep(10) # BATCH UPLOAD EVERY 10 SECONDS
                    if cls._dirty:
                        try:
...
class AdminBot:
    ...
    async def _poll_loop(self) -> None:
        ...
        while True:
            ...
            except Exception as exc: await asyncio.sleep(3)
    ...
    def _build_stats(self, chat_id: int) -> str:
        ...
        return f"📊 <b>Farm Analytics Report</b>\n━━━━━━━━━━━━━━━━━━━━━━\n📅 Today's Profit: <code>{today_profit:,} Extols</code>\n🗓 Last 7 Days: <code>{week_profit:,} Extols</code>\n🏆 Top Farmer: <code>{top_acc_name}</code> ({top_acc_profit:,})\n👥 Active Accounts: <code>{len(visible_accs)}</code>\n━━━━━━━━━━━━━━━━━━━━━━"
    ...
    def _build_guess(self, chat_id: int) -> str:
        ...
        if len(active_profits) < 2: return "🤖 <b>AI Analysis:</b>\nInsufficient historical data. Please let the bot farm for at least 2 days to generate a trend model."
        ...
        return f"🤖 <b>Smart ML Prediction Model</b>\n━━━━━━━━━━━━━━━━━━━━━━\n⚡️ Active Avg Speed: <code>{int(weighted_avg):,} Extols/day</code>\n📈 Trend Momentum: <code>{trend_emoji}</code>\n\n🔮 1 Week Forecast: <code>{int(week_pred):,} Extols</code>\n🔮 1 Month Forecast: <code>{int(month_pred):,} Extols</code>\n\n<i>(Modeled via Weighted Regression Math)</i>"
        
    def _build_menu(self, chat_id: int) -> str:
        settings = self.store.settings(); my_accs = len(self.get_visible_accounts(chat_id))
        return f"⚙️ <b>SlotOps Admin Panel</b>\n━━━━━━━━━━━━━━━━━━━━━━\n⚡ <b>Status:</b> <code>{'Running' if settings['automation_enabled'] else 'Paused'}</code>\n👤 <b>Accounts:</b> <code>{my_accs}</code>\n━━━━━━━━━━━━━━━━━━━━━━\n🎮 <b>Choose an option below:</b>"
    ...
    async def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        try:
            ...
            elif data.startswith("delete_account_confirm:"): account = self.store.resolve_account(data.split(":", 1)[1]); await self.reply(chat_id, f"❌ <b>Delete account?</b>\n\n<code>{account['label']}</code>", delete_confirm_keyboard(account["id"]), message_id=message_id)
            elif data.startswith("delete_account_yes:"): account = self.store.resolve_account(data.split(":", 1)[1]); self.store.delete_account(account["id"]); CustomDB.set(f"owner_{account['id']}", None); await self.reply(chat_id, f"✅ <b>Deleted:</b> <code>{account['label']}</code>", accounts_keyboard(), message_id=message_id)
            elif data == "delete_all_accounts_confirm": await self.reply(chat_id, "❌ <b>WARNING</b>\nDelete <b>ALL YOUR ACCOUNTS</b>?", inline([[button("✅ Yes, Delete All", "delete_all_accounts_yes")], [button("🔙 Cancel", "accounts")]]), message_id=message_id)
            ...
            elif data.startswith("delete_group_confirm:"): group = self.store.resolve_group(data.split(":", 1)[1]); await self.reply(chat_id, f"❌ <b>Delete group?</b>\n\n<code>{group['title']}</code>", delete_group_confirm_keyboard(group["id"]), message_id=message_id)
            elif data.startswith("delete_group_yes:"): group = self.store.resolve_group(data.split(":", 1)[1]); self.store.delete_group(group["id"]); await self.reply(chat_id, f"✅ <b>Deleted:</b> <code>{group['title']}</code>", groups_keyboard(), message_id=message_id)
            ...
            elif data == "start_auto": self.store.update_settings({"automation_enabled": True}); await self.reply(chat_id, "✅ Automation started.", main_keyboard(), message_id=message_id)
            elif data == "pause_auto": self.store.update_settings({"automation_enabled": False}); await self.reply(chat_id, "❌ Automation paused.", main_keyboard(), message_id=message_id)
            ...
            elif data == "slot_schedule_help": await self.reply(chat_id, "⏱ <b>Slot Schedule</b>\nSend:\n<code>/set_slot /slot | 12 | 8 | 12</code>", settings_keyboard(), message_id=message_id)
            ...
            elif data == "action_log": self.store.update_settings({"action": "log_only"}); await self.reply(chat_id, "✅ Action: log only.", settings_keyboard(), message_id=message_id)
        ...
        except Exception as exc: await self.reply(chat_id, f"❌ Error: {exc}", main_keyboard(), message_id=message_id)

    async def shift_admin(self, chat_id: int, args: str) -> None:
        if chat_id not in self.owner_admin_ids: await self.reply(chat_id, "❌ Access Denied"); return
        if not args: await self.reply(chat_id, "ℹ️ Usage:\n<code>/shift ADMIN_ID</code>"); return
        if args.lower() in ["reset", "me"]: CustomDB.set(f"shift_{chat_id}", str(chat_id)); await self.reply(chat_id, "✅ Shift Reset", main_keyboard())
        elif args.lower() == "all": CustomDB.set(f"shift_{chat_id}", "all"); await self.reply(chat_id, "👤 Master View Active", main_keyboard())
        else: CustomDB.set(f"shift_{chat_id}", args.strip()); await self.reply(chat_id, f"👤 Impersonating Admin: <code>{args}</code>", main_keyboard())
    ...
    async def server_status(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        text = "⚙️ <b>Server Status</b>\n━━━━━━━━━━━━━━━━━━━━━━\n⏱ Uptime: <code>{}h {}m {}s</code>\n🤖 RAM Usage: <code>{}</code>\n🔗 Active Tasks: <code>{}</code>\n👤 Your Accounts: <code>{}</code>\n━━━━━━━━━━━━━━━━━━━━━━".format(h, m, s, ram_usage, len(self.active_tasks), len(self.get_visible_accounts(chat_id)))
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        for grp in self.store.groups_for_account(acc["id"]):
                            try: await client.send_message(grp["identifier"], message_text); success_count += 1; await self.reply(chat_id, f"✅ Claim sent from: {acc['label']} ➡️ {grp['title']}")
                            except Exception: pass
                    await client.disconnect()
                except asyncio.CancelledError: raise
                except Exception as e: await self.reply(chat_id, f"❌ Failed from {acc['label']}: {str(e)}")
            if chat_id in self.active_tasks: del self.active_tasks[chat_id]
            await self.reply(chat_id, f"✅ Claim Finished! Total Success: {success_count} messages sent.")
    ...
    async def execute_mass_message(self, chat_id: int, target: str, message_text: str, count: int) -> None:
        try:
            ...
            if not my_accounts: await self.reply(chat_id, "❌ No accounts available."); return
            random.shuffle(my_accounts); selected_accounts = my_accounts[:count]
            await self.reply(chat_id, f"🔗 Initiating Mass Message to {target}...\n👤 Total Accounts to be used: {len(selected_accounts)}")
            success_count = 0
            for i, acc in enumerate(selected_accounts):
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        ...
                        await client.send_message(actual_target, message_text); success_count += 1; await self.reply(chat_id, f"✅ Message sent from: {acc['label']}")
                    await client.disconnect()
                except asyncio.CancelledError: raise
                except Exception as e: await self.reply(chat_id, f"❌ Failed from {acc['label']}: {str(e)}")
            if chat_id in self.active_tasks: del self.active_tasks[chat_id]
            await self.reply(chat_id, f"✅ Finished! Successfully sent from {success_count}/{len(selected_accounts)} accounts.")
        except asyncio.CancelledError: pass
        except Exception as e: await self.reply(chat_id, f"❌ Error: {str(e)}")
    ...
    async def handle_command(self, chat_id: int, text: str) -> None:
        ...
        try:
            if chat_id in MESSAGE_STATE:
                ...
                if text == "/cancel": del MESSAGE_STATE[chat_id]; await self.reply(chat_id, "❌ Cancelled.", main_keyboard()); return
                if state["step"] == "waiting_target": state["target"] = text.strip(); state["step"] = "waiting_message"; await self.reply(chat_id, f"✅ Target set: {state['target']}\n\n🔗 Step 2: What message do you want to send?"); return
                elif state["step"] == "waiting_message": state["message"] = text.strip(); state["step"] = "waiting_count"; await self.reply(chat_id, f"✅ Message saved.\n\n👤 Step 3: How many accounts should send this message? (Enter a number or 'all')"); return
                    ...
                    if count_str == 'all': count = total_visible
                        ...
                        except ValueError: await self.reply(chat_id, "❌ Enter a valid number, 'all', or /cancel"); return
                    target = state["target"]; msg_text = state["message"]; del MESSAGE_STATE[chat_id]
                    await self.reply(chat_id, f"✅ Setup Complete! Launching mass message task to {count} accounts...")
            ...
            if command in {"/start", "/admin"}: await self.send_menu(chat_id)
            elif command == "/message": MESSAGE_STATE[chat_id] = {"step": "waiting_target"}; await self.reply(chat_id, "🔗 Step 1: Send the target Username or Group Link (e.g., @username or https://t.me/...)")
            elif command == "/claim":
                if not args: await self.reply(chat_id, "❌ Usage: <code>/claim MESSAGE</code>\nExample: <code>/claim /daily</code>", main_keyboard()); return
                my_accounts = self.get_visible_accounts(chat_id)
                if not my_accounts: await self.reply(chat_id, "❌ No accounts available to claim."); return
                await self.reply(chat_id, f"🔗 Initiating Mass Claim...\nSending <code>{args}</code> to assigned groups for {len(my_accounts)} accounts.")
            ...
            elif command == "/accounts": await self.reply(chat_id, await asyncio.to_thread(self.render_accounts, chat_id), accounts_keyboard())
            elif command in {"/bal", "/balance", "/balances"}: await self.reply(chat_id, await asyncio.to_thread(self.render_balances, chat_id), main_keyboard())
            elif command == "/groups": await self.reply(chat_id, await asyncio.to_thread(self.render_groups), groups_keyboard())
            elif command == "/assignments": await self.reply(chat_id, await asyncio.to_thread(self.render_assignments, chat_id), assignment_home_keyboard(self.get_visible_accounts(chat_id)))
                ...
                await self.reply(chat_id, f"✅ Balance updated for <code>{found_acc['label']}</code>: {bal} Extols", main_keyboard())
            elif command == "/set_balance_group":
                target = args.strip()
                if not target: raise ValueError("Usage: /set_balance_group @GroupLink")
                CustomDB.set("balance_group_target", target); await self.reply(chat_id, f"✅ Balance forwarding group set to: <b>{target}</b>", main_keyboard())
                ...
                grps = self.store.groups()
                grp = next((g for g in grps if g["title"].lower() == parts[1].lower() or str(g.get("identifier","")).lower() == parts[1].lower()), None) or next((g for g in grps if g["id"].lower().startswith(parts[1].lower())), None)
                if acc and grp: CustomDB.set(f"target_{acc['id']}_{grp['id']}", parts[2]); await self.reply(chat_id, "✅ Target Time set", main_keyboard())
                ...
                if acc and grp: self.store.unassign_group(acc["id"], grp["id"]); await self.reply(chat_id, "❌ Removed.", main_keyboard())
            elif command == "/start_auto": self.store.update_settings({"automation_enabled": True}); await self.reply(chat_id, "✅ Started.", main_keyboard())
            elif command == "/pause_auto": self.store.update_settings({"automation_enabled": False}); await self.reply(chat_id, "❌ Paused.", main_keyboard())
            ...
            elif command == "/limit": CustomDB.set("limit", int(args.strip())); await self.reply(chat_id, "✅ Limit updated.", settings_keyboard())
                ...
                self.store.delete_admin_id(admin_id); self.sync_admin_ids(); await self.reply(chat_id, "❌ Removed.", admins_keyboard())
    ...
    async def show_account_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        if not accounts: await self.reply(chat_id, "❌ No accounts.", accounts_keyboard(), message_id=message_id); return
        rows, row = [], []
    ...
    async def show_delete_group_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        if not groups: await self.reply(chat_id, "❌ No groups.", groups_keyboard(), message_id=message_id); return
        rows, row = [], []
        for g in groups[:100]:
    ...
    async def assign_ids(self, chat_id: int, account_id: str, group_id: str, message_id: int | None = None) -> None:
        ...
        await self.reply(chat_id, "✅ Assigned successfully.", main_keyboard(), message_id=message_id)

    def render_accounts(self, chat_id: int) -> str:
        accounts = self.get_visible_accounts(chat_id)
        if not accounts: return "❌ No accounts in your panel."
    ...
    def render_groups(self) -> str:
        groups = self.store.groups()
        if not groups: return "❌ No groups."
    ...
    def render_assignments(self, chat_id: int) -> str:
        ...
        try:
            ...
            for r in raw_assigns:
                if r['account_id'] in my_acc_ids:
                    lines.append(f"👤 <code>{accounts_map.get(r['account_id'], {}).get('label', 'Unknown')}</code> ➡️ 📂 <b>{all_groups.get(r['group_id'], {}).get('title', 'Unknown')}</b> [{CustomDB.get(f'target_{r['account_id']}_{r['group_id']}', 'Jitter')}]")
        except Exception:
        ...
        return "\n".join(lines) if len(lines) > 1 else "❌ No assignments for your accounts."
