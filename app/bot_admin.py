...
# 🔥 REAL NITRO BOOST: Background Daemon Caching 🔥
class CustomDB:
    ...
    def _refresh_cache(cls):
        now = time.time()
        # Agar cache purana ho gaya hai, toh turant time update karo taaki doosre click na fasen
        if now - cls._cache_time > 10:
            cls._cache_time = now 
            
            # Fetching ka saara kaam background thread me daal diya taaki bot freeze na ho!
...
━━━━━━━━━━━━━━━━━━━━━━"""
        ...
        while True:
            try:
                ...
                for update in updates:
                    ...
                    # Task ko parallel start karo taaki agla click na ruke!
    ...
    async def handle_update(self, update: dict[str, Any]) -> None:
        ...
        # Command aane par freeze na ho isliye task bana diya
        asyncio.create_task(self.handle_command(chat_id, text))

    # --- THREADED BUILDERS (Ye saara heavy data fetching background me karenge) ---
    ...
    def _build_stats(self, chat_id: int) -> str:
        ...
        return (
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📅 **Aaj Ki Kamai:** `{today_profit:,} Extols`\n"
            f"🗓️ **Pichle 7 Din:** `{week_profit:,} Extols`\n"
            f"🏆 **Top Farmer (Aaj):** `{top_acc_name}` ({top_acc_profit:,} Extols)\n"
            f"👥 **Active Accounts:** `{len(visible_accs)}`\n"
            "━━━━━━━━━━━━━━━━━━━━━━"
        )
    ...
    def _build_guess(self, chat_id: int) -> str:
        ...
        if days_counted == 0:
            return "🤖 **AI Analysis:**\nBhai abhi tak DB mein pichle dino ka data nahi mila. Thoda farm hone do phir prediction karunga!"
        ...
        return (
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚡ **Average Speed:** `{avg_daily:,} Extols/day`\n\n"
            f"🔮 **1 Week Prediction:** `{week_pred:,} Extols`\n"
            f"🔮 **1 Month Prediction:** `{month_pred:,} Extols`\n\n"
            "*(Ye estimate pichle dino ki speed par based hai)* 🚀"
        )
    ...
    # 🔥 FIX 2: Async Threaded Callbacks (No more Clock Loading Icon Delay!) 🔥
    async def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        # Instantly tell Telegram that click is received (removes loading icon instantly)
        asyncio.create_task(self.answer_callback(query_id))

        try:
            # Har heavy loading wale function ko hum await asyncio.to_thread karenge
            if data == "menu": 
    ...
    async def shift_admin(self, chat_id: int, args: str) -> None:
        if chat_id not in self.owner_admin_ids:
            await self.reply(chat_id, "❌ <b>Access Denied</b>"); return
        if not args:
            await self.reply(chat_id, "ℹ️ <b>Usage:</b>\n<code>/shift ADMIN_ID</code>"); return
        if args.lower() in ["reset", "me"]:
            CustomDB.set(f"shift_{chat_id}", str(chat_id)); await self.reply(chat_id, "🔙 <b>Shift Reset</b>", main_keyboard())
        elif args.lower() == "all":
            CustomDB.set(f"shift_{chat_id}", "all"); await self.reply(chat_id, "🌐 <b>Master View Active</b>", main_keyboard())
        else:
            CustomDB.set(f"shift_{chat_id}", args.strip()); await self.reply(chat_id, f"👁️ <b>Impersonating Admin:</b> <code>{args}</code>", main_keyboard())
    ...
    async def server_status(self, chat_id: int) -> None:
        ...
        text = f"🖥️ <b>Server Status</b> 🖥️\n━━━━━━━━━━━━━━━━━━━━━━\n⏱️ <b>Uptime:</b> <code>{h}h {m}m {s}s</code>\n🧠 <b>RAM Usage:</b> <code>{ram_usage}</code>\n⚡ <b>Active Tasks:</b> <code>{len(self.active_tasks)}</code>\n👤 <b>Your Accounts:</b> <code>{len(self.get_visible_accounts(chat_id))}</code>\n━━━━━━━━━━━━━━━━━━━━━━"
    ...
    async def handle_command(self, chat_id: int, text: str) -> None:
        ...
        try:
            ...
            if command in {"/start", "/admin"}: await self.send_menu(chat_id)
                ...
                await self.reply(chat_id, "💸 <b>Limit updated.</b>", settings_keyboard())
    ...
    async def answer_callback(self, query_id: str, text: str | None = None) -> None:
        ...
        def do_answer():
            try:
                with urllib.request.urlopen(urllib.request.Request(f"https://api.telegram.org/bot{self.notifier.bot_token}/answerCallbackQuery", data=urllib.parse.urlencode(data).encode("utf-8"), headers={"Content-Type": "application/x-www-form-urlencoded"}, method="POST"), timeout=5): return
            except Exception: pass
...
def main_keyboard() -> dict:
    return inline([
        ...
        [button("📊 Stats & Analytics", "stats"), button("🤖 AI Guess", "guess")],
