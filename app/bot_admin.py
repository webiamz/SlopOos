...
⚙️ <b>Assignments & Forwarding:</b>
<code>/assign ACC GROUP [HH:MM]</code>
<code>/set_time ACC GROUP HH:MM</code>
<code>/set_balance_group @GroupLink</code> (Set where to forward Zoro replies)
...
━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    async def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        try:
            ...
            elif data == "balances": await self.reply(chat_id, "ℹ️ Balance tracking disabled. Check forward group.", main_keyboard(), message_id=message_id)
    ...
    async def handle_command(self, chat_id: int, text: str) -> None:
        ...
        try:
            ...
            elif command in {"/bal", "/balance", "/balances"}: await self.reply(chat_id, "ℹ️ Balance tracking is handled via forwarded messages.", main_keyboard())
            ...
            # 🔥 NEW: Set Forwarding Target Group 🔥
            ...
            # 🔥 UPDATED: Add Account + Auto Join 🔥
                ...
                acc, grp = self.store.resolve_account(parts[0]), self.store.resolve_group(parts[1])
...
def settings_keyboard() -> dict: return inline([[button("📝 Log Only", "action_log"), button("📢 Auto Msg", "action_send_help")], [button("🔄 Cycle", "cycle_help")], [button("🏠 Back to Main", "menu")]])
