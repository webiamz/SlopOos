...
class AdminBot:
    ...
    def get_help_text(self) -> str:
        return f"""{E_HELP} <b>SlotOps Admin Guide</b>
━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    def _build_stats(self, chat_id: int) -> str:
        ...
        return (
            f"{E_STATS} <b>Farm Analytics Report</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{E_TIME} Today's Profit: <code>{today_profit:,} Extols</code>\n"
            f"{E_TIME} Last 7 Days: <code>{week_profit:,} Extols</code>\n"
            f"🏆 Top Farmer: <code>{top_acc_name}</code> ({top_acc_profit:,})\n"
            f"{E_USER} Active Accounts: <code>{len(visible_accs)}</code>\n"
            "━━━━━━━━━━━━━━━━━━━━━━"
        )
    ...
    def _build_guess(self, chat_id: int) -> str:
        ...
        return (
            f"{E_AI} <b>Smart ML Prediction Model</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{E_TIME} Active Avg Speed: <code>{int(weighted_avg):,} Extols/day</code>\n"
            f"{E_STATS} Trend Momentum: <code>{trend_emoji}</code>\n\n"
            f"🔮 1 Week Forecast: <code>{int(week_pred):,} Extols</code>\n"
    ...
    def _build_menu(self, chat_id: int) -> str:
        ...
        return (f"{E_SET} <b>SlotOps Admin Panel</b>\n━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"{E_USER} <b>Accounts:</b> <code>{my_accs}</code>\n━━━━━━━━━━━━━━━━━━━━━━\n🎮 <b>Choose an option below:</b>")
    ...
    async def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        try:
            if data == "menu": 
                ...
                await self.server_status(chat_id, message_id=message_id)
            elif data == "groups": 
                ...
                await self.reply(chat_id, f"{E_ERR} <b>Delete group?</b>\n\n<code>{group['title']}</code>", delete_group_confirm_keyboard(group["id"]), message_id=message_id)
            elif data.startswith("delete_group_yes:"):
                group = self.store.resolve_group(data.split(":", 1)[1])
                self.store.delete_group(group["id"])
                await self.reply(chat_id, f"{E_CHK} <b>Deleted:</b> <code>{group['title']}</code>", groups_keyboard(), message_id=message_id)
                ...
                await self.reply(chat_id, f"{E_CHK} Automation started.", main_keyboard(), message_id=message_id)
                ...
                await self.reply(chat_id, f"Airdrop Auto-Trigger is now **{new_status}**.", main_keyboard(), message_id=message_id)
                ...
                if len(parts) == 3:
                    ...
                    if account:
                        try:
                            ...
                            await self.reply(chat_id, f"{E_CHK} <b>Session Terminated!</b>\nThe intruder has been successfully logged out from <code>{account['label']}</code>.", message_id=message_id)
                        except Exception as e:
                            await self.reply(chat_id, f"{E_ERR} Failed to kick session. Error: {e}", message_id=message_id)
                ...
                if len(parts) == 3:
                    ...
                    await self.reply(chat_id, f"{E_CHK} <b>Login Approved!</b>\nAccess granted for <code>{label}</code>.", message_id=message_id)

            elif data == "add_admin_help": 
                await self.reply(chat_id, f"{E_USER} <b>Add Admin</b>\nSend:\n<code>/add_admin USER_ID</code>", admins_keyboard(), message_id=message_id)
            elif data == "delete_admin_help": 
                await self.reply(chat_id, f"{E_ERR} <b>Remove Admin</b>\nSend:\n<code>/del_admin USER_ID</code>", admins_keyboard(), message_id=message_id)
            elif data == "slot_schedule_help": 
                await self.reply(chat_id, f"{E_TIME} <b>Slot Schedule</b>\nSend:\n<code>/set_slot /slot | 12 | 8 | 12</code>", settings_keyboard(), message_id=message_id)
            elif data == "test_send_help": 
                await self.reply(chat_id, f"{E_SET} <b>Test Message</b>\nSend:\n<code>/test_send ACCOUNT GROUP | test msg</code>", settings_keyboard(), message_id=message_id)
            elif data == "action_log":
                self.store.update_settings({"action": "log_only"})
                await self.reply(chat_id, f"{E_CHK} Action: log only.", settings_keyboard(), message_id=message_id)
            elif data == "action_send_help": 
                await self.reply(chat_id, f"{E_LINK} <b>Auto Message</b>\nSend:\n<code>/set_action send_message | Response text</code>", settings_keyboard(), message_id=message_id)
            elif data == "cycle_help": 
                await self.reply(chat_id, f"{E_TIME} <b>Set Cycle</b>\nSend:\n<code>/set_cycle 12</code>", settings_keyboard(), message_id=message_id)
            elif data == "keywords_help": 
                await self.reply(chat_id, f"{E_SEC} <b>Set Keywords</b>\nSend:\n<code>/set_keywords slot,booking</code>", settings_keyboard(), message_id=message_id)
            elif data == "help": 
                await self.reply(chat_id, self.get_help_text(), main_keyboard(), message_id=message_id)
            else: 
                await self.reply(chat_id, f"{E_ERR} Unknown button.", main_keyboard(), message_id=message_id)
        except Exception as exc:
            await self.reply(chat_id, f"{E_ERR} Error: {exc}", main_keyboard(), message_id=message_id)

    async def shift_admin(self, chat_id: int, args: str) -> None:
        if chat_id not in self.owner_admin_ids:
            await self.reply(chat_id, f"{E_ERR} Access Denied"); return
        if not args:
            await self.reply(chat_id, f"{E_INFO} Usage:\n<code>/shift ADMIN_ID</code>"); return
        if args.lower() in ["reset", "me"]:
            CustomDB.set(f"shift_{chat_id}", str(chat_id)); await self.reply(chat_id, f"{E_CHK} Shift Reset", main_keyboard())
        elif args.lower() == "all":
            CustomDB.set(f"shift_{chat_id}", "all"); await self.reply(chat_id, f"{E_USER} Master View Active", main_keyboard())
        else:
            CustomDB.set(f"shift_{chat_id}", args.strip()); await self.reply(chat_id, f"{E_USER} Impersonating Admin: <code>{args}</code>", main_keyboard())
            ...
            for i, acc in enumerate(accounts):
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        assigned_groups = self.store.groups_for_account(acc["id"])
                        for grp in assigned_groups:
                            try:
                                actual_target = grp["identifier"]
                                await client.send_message(actual_target, message_text)
                                success_count += 1
                                await self.reply(chat_id, f"{E_CHK} Claim sent from: {acc['label']} ➡️ {grp['title']}")
                            except Exception:
                                pass
    ...
    async def execute_mass_message(self, chat_id: int, target: str, message_text: str, count: int) -> None:
        try:
            ...
            if not my_accounts:
                await self.reply(chat_id, f"{E_ERR} No accounts available.")
                return

            random.shuffle(my_accounts)
            selected_accounts = my_accounts[:count]
            ...
            for i, acc in enumerate(selected_accounts):
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        ...
                        await client.send_message(actual_target, message_text)
                        success_count += 1
                        await self.reply(chat_id, f"{E_CHK} Message sent from: {acc['label']}")
                    await client.disconnect()
                except asyncio.CancelledError: raise
                except Exception as e: await self.reply(chat_id, f"{E_ERR} Failed from {acc['label']}: {str(e)}")
    ...
    async def handle_command(self, chat_id: int, text: str) -> None:
        ...
        try:
            if text.strip().isdigit() and len(text.strip()) <= 3:
                pending_acc = CustomDB.get("pending_captcha_acc")
                if pending_acc:
                    CustomDB.set("admin_captcha_input", text.strip())
                    await self.reply(chat_id, f"{E_CHK} Received Answer: <b>{text.strip()}</b>\nBot is pressing the button...", main_keyboard())
                    return

            if chat_id in MESSAGE_STATE:
                state = MESSAGE_STATE[chat_id]
                if text == "/cancel":
                    del MESSAGE_STATE[chat_id]
                ...
                if state["step"] == "waiting_target":
                    ...
                    await self.reply(chat_id, f"{E_CHK} Target set: {state['target']}\n\n{E_LINK} Step 2: What message do you want to send?")
                    return

                elif state["step"] == "waiting_message":
                    state["message"] = text.strip()
                    state["step"] = "waiting_count"
                    await self.reply(chat_id, f"{E_CHK} Message saved.\n\n{E_USER} Step 3: How many accounts should send this message? (Enter a number or 'all')")
                    return

                elif state["step"] == "waiting_count":
                    count_str = text.strip().lower()
                    total_visible = len(self.get_visible_accounts(chat_id))
                    if count_str == 'all':
                        count = total_visible
                    else:
                        try:
                            count = int(count_str)
                            if count > total_visible: count = total_visible
                        except ValueError:
                            await self.reply(chat_id, f"{E_ERR} Enter a valid number, 'all', or /cancel")
                            return
            ...
            if command in {"/start", "/admin"}: await self.send_menu(chat_id)
                ...
                MESSAGE_STATE[chat_id] = {"step": "waiting_target"}
                await self.reply(chat_id, f"{E_LINK} Step 1: Send the target Username or Group Link (e.g., @username or https://t.me/...)")
                
            elif command == "/claim":
                if not args:
                    await self.reply(chat_id, f"{E_ERR} Usage: <code>/claim MESSAGE</code>\nExample: <code>/claim /daily</code>", main_keyboard())
                    return
                
                my_accounts = self.get_visible_accounts(chat_id)
                if not my_accounts:
                    await self.reply(chat_id, f"{E_ERR} No accounts available to claim.")
                    return
                ...
                msg = f"{E_CHK} Balance updated for <code>{found_acc['label']}</code>: {bal} Extols"
                ...
                await self.reply(chat_id, f"{E_CHK} Balance forwarding group set to: <b>{target}</b>", main_keyboard())
                ...
                if acc and grp:
                    ...
                    await self.reply(chat_id, f"{E_CHK} Target Time set", main_keyboard())
                ...
                if acc and grp:
                    ...
                    await self.reply(chat_id, f"{E_ERR} Removed.", main_keyboard())
                ...
                await self.reply(chat_id, f"{E_CHK} Limit updated.", settings_keyboard())
            ...
            else: await self.reply(chat_id, f"{E_ERR} Unknown Command.", main_keyboard())
    ...
    async def show_account_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        await self.reply(chat_id, f"👇 Select account:", inline(rows), message_id=message_id)
    ...
    async def show_delete_account_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        await self.reply(chat_id, f"👇 Select account to delete:", inline(rows), message_id=message_id)
    ...
    async def show_delete_group_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        await self.reply(chat_id, f"👇 Select group to delete:", inline(rows), message_id=message_id)
    ...
    def render_balances(self, chat_id: int) -> str:
        ...
        total = 0
        lines = [f"{E_MONEY} <b>Live Extols Balance</b>\n━━━━━━━━━━━━━━━━━━━━━━"]
        for a in accounts:
        ...
        try:
            ...
            for r in raw_assigns:
                if r['account_id'] in my_acc_ids:
                    ...
                    lines.append(f"{E_USER} <code>{acc_label}</code> ➡️ {E_GRP} <b>{grp_title}</b> [{CustomDB.get(f'target_{r['account_id']}_{r['group_id']}', 'Jitter')}]")
        except Exception:
...
def main_keyboard() -> dict:
    airdrop_status = CustomDB.get("airdrop_enabled", False)
    airdrop_text = "🎁 Airdrop: ON 🟢" if airdrop_status else "🎁 Airdrop: OFF 🔴"
    ...
    return inline([
        ...
        [button(airdrop_text, "toggle_airdrop")],
