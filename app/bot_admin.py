...
def bottom_tiles() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton("👤 Accounts"), KeyboardButton("💰 Balances")],
            [KeyboardButton("📈 Stats"), KeyboardButton("⚙️ Settings")],
            [KeyboardButton("🔗 Assignments"), KeyboardButton("ℹ️ Help")]
        ],
...
def main_keyboard() -> InlineKeyboardMarkup:
    return inline([
        [button("👤 Accounts", "accounts", "5258362837411045098"), button("💰 Balances", "balances")],  # 🧪 TEST: sirf Accounts pe icon
...
def accounts_keyboard() -> InlineKeyboardMarkup: return inline([[button("➕ Add Account", "add_account_help"), button("❌ Delete Account", "delete_account_start")], [button("⚠️ Delete ALL My Accounts ⚠️", "delete_all_accounts_confirm")], [button("🔗 Assign Account", "assign_start"), button("🏠 Back to Main", "menu")]])
def groups_keyboard() -> InlineKeyboardMarkup: return inline([[button("➕ Add Group", "add_group_help"), button("❌ Delete Group", "delete_group_start")], [button("🏠 Back to Main", "menu")]])
def assignment_home_keyboard(accounts: list) -> InlineKeyboardMarkup: return inline([[button("🔗 Assign Account", "assign_start")], [button("👤 Accounts", "accounts"), button("📂 Groups", "groups")], [button("🏠 Back to Main", "menu")]]) if accounts else inline([[button("➕ Add Account", "add_account_help")], [button("🏠 Back to Main", "menu")]])
def settings_keyboard() -> InlineKeyboardMarkup: return inline([[button("📝 Log Only", "action_log"), button("📢 Auto Msg", "action_send_help")], [button("🔄 Cycle", "cycle_help"), button("💸 Limit", "limit_help")], [button("🏠 Back to Main", "menu")]])
def admins_keyboard() -> InlineKeyboardMarkup: return inline([[button("➕ Add Admin", "add_admin_help"), button("❌ Remove Admin", "delete_admin_help")], [button("🏠 Back to Main", "menu")]])
def back_keyboard() -> InlineKeyboardMarkup: return inline([[button("🔙 Back", "menu")]])
def delete_confirm_keyboard(account_id: str) -> InlineKeyboardMarkup: return inline([[button("✅ Yes Delete", f"delete_account_yes:{short(account_id)}")], [button("🔙 Cancel", "accounts")]])
def delete_group_confirm_keyboard(group_id: str) -> InlineKeyboardMarkup: return inline([[button("✅ Yes Delete", f"delete_group_yes:{short(group_id)}")], [button("🔙 Cancel", "groups")]])
...
def inline(rows: list) -> InlineKeyboardMarkup:
    ...
    for row in rows:
        ...
        for btn in row:
            ...
            buttons.append(InlineKeyboardButton(btn["text"], **kwargs))
        kbd.append(buttons)
    return InlineKeyboardMarkup(kbd)

def button(text: str, callback_data: str, icon_emoji_id: str | None = None) -> dict:
    b = {"text": text, "callback_data": callback_data}
    if icon_emoji_id: b["icon_custom_emoji_id"] = icon_emoji_id
    return b
