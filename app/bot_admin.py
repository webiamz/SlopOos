...
E_PIN = ce("📌", "5258461531464539536")
E_EYE = ce("👁", "5253959125838090076")
E_GAME = ce("🎮", "5258508428212445001")
E_ARROW = ce("➡️", "5258215850745275216")
...
class AdminBot:
    ...
    async def show_delete_account_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        rows, row = [], []
        for a in accounts[:100]:
            row.append(button(f"❌ {a['label']}", f"delete_account_confirm:{short(a['id'])}", style="danger"))
            if len(row) == 2: rows.append(row); row = []
        if row: rows.append(row)
    ...
    async def show_delete_group_picker(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        rows, row = [], []
        for g in groups[:100]:
            row.append(button(f"❌ {g['title']}", f"delete_group_confirm:{short(g['id'])}", style="danger"))
            if len(row) == 2: rows.append(row); row = []
        if row: rows.append(row)
    ...
    async def reply(self, chat_id: int, text: str, markup: dict | None = None, message_id: int | None = None) -> None:
        try:
            ...
            try:
                await self.app.send_message(chat_id, f"⚠️ Panel error: {safe_err}")
            except Exception:
                pass
...
# 🎨 COLORED TELEGRAM BUTTONS (KURIGRAM / MTProto) 🎨
# Telegram's native button styles are:
#   primary = blue, success = green, danger = red.
# Kurigram exposes the underlying MTProto constructors through pyrogram.raw.
#
# We intentionally build the markup with raw MTProto types instead of passing
# style= to Pyrogram's old high-level InlineKeyboardButton/KeyboardButton
# wrappers, because those wrappers may not expose the new style field yet.

def _button_style(kind: str = "primary", icon_emoji_id: str | None = None):
    ...
    if icon_emoji_id:
        kwargs["icon"] = int(icon_emoji_id)
    return raw.types.KeyboardButtonStyle(**kwargs)


def _raw_callback_button(
    text: str,
    callback_data: str,
    style: str = "primary",
    ...
    return raw.types.KeyboardButtonCallback(
        text=text,
        data=str(callback_data).encode("utf-8"),
        style=_button_style(style, icon_emoji_id),
    )


def bottom_tiles() -> ReplyKeyboardMarkup:
    # Bottom persistent keyboard: mostly blue for a clean navigation bar,
    # with Balances highlighted green.
    rows = [
        [
    ...
    return raw.types.ReplyKeyboardMarkup(
        rows=[raw.types.KeyboardButtonRow(buttons=row) for row in rows],
        resize=True,
        persistent=True,
    )
...
def main_keyboard() -> InlineKeyboardMarkup:
    return inline([
        [
            button("👤 Accounts", "accounts", "5258362837411045098", style="primary"),
            button("💰 Balances", "balances", style="success"),
        ],
        [
            button("📈 Stats", "stats", style="primary"),
            button("🤖 AI Guess", "guess", style="primary"),
            button("🖥️ Status", "status", style="primary"),
        ],
        [
            button("🔗 Assign Account", "assign_start", style="primary"),
            button("🚀 Assignments", "assignments", style="primary"),
        ],
        [
            button("⚡ Start Auto", "start_auto", style="success"),
            button("🔥 Pause Auto", "pause_auto", style="danger"),
        ],
        [
            button("⚙️ Settings", "settings", style="primary"),
            button("👤 Admins", "admins", style="primary"),
        ],
        [
            button("❓ Help & Guide", "help", style="primary"),
        ],
    ])
...
def accounts_keyboard() -> InlineKeyboardMarkup:
    return inline([
        [
            button("➕ Add Account", "add_account_help", style="success"),
            button("❌ Delete Account", "delete_account_start", style="danger"),
        ],
        [
            button("⚠️ Delete ALL My Accounts ⚠️", "delete_all_accounts_confirm", style="danger"),
        ],
        [
            button("🔗 Assign Account", "assign_start", style="primary"),
            button("🏠 Back to Main", "menu", style="primary"),
        ],
    ])
...
def groups_keyboard() -> InlineKeyboardMarkup:
    return inline([
        [
            button("➕ Add Group", "add_group_help", style="success"),
            button("❌ Delete Group", "delete_group_start", style="danger"),
        ],
        [
            button("🏠 Back to Main", "menu", style="primary"),
        ],
...
def assignment_home_keyboard(accounts: list) -> InlineKeyboardMarkup:
    return inline(
        [
            [button("🔗 Assign Account", "assign_start", style="primary")],
            [
                button("👤 Accounts", "accounts", style="primary"),
                button("📂 Groups", "groups", style="primary"),
            ],
            [button("🏠 Back to Main", "menu", style="primary")],
        ]
        if accounts
...
def settings_keyboard() -> InlineKeyboardMarkup:
    return inline([
        [
            button("📝 Log Only", "action_log", style="primary"),
            button("📢 Auto Msg", "action_send_help", style="primary"),
        ],
        [
            button("🔄 Cycle", "cycle_help", style="primary"),
            button("💸 Limit", "limit_help", style="primary"),
        ],
        [
            button("🏠 Back to Main", "menu", style="primary"),
        ],
...
def admins_keyboard() -> InlineKeyboardMarkup:
    return inline([
        [
            button("➕ Add Admin", "add_admin_help", style="success"),
            button("❌ Remove Admin", "delete_admin_help", style="danger"),
        ],
        [
            button("🏠 Back to Main", "menu", style="primary"),
        ],
...
def back_keyboard() -> InlineKeyboardMarkup:
    return inline([[button("🔙 Back", "menu", style="primary")]])


def delete_confirm_keyboard(account_id: str) -> InlineKeyboardMarkup:
    return inline([
        [button("✅ Yes Delete", f"delete_account_yes:{short(account_id)}", style="danger")],
        [button("🔙 Cancel", "accounts", style="primary")],
    ])


def delete_group_confirm_keyboard(group_id: str) -> InlineKeyboardMarkup:
    return inline([
        [button("✅ Yes Delete", f"delete_group_yes:{short(group_id)}", style="danger")],
        [button("🔙 Cancel", "groups", style="primary")],
    ])
...
def inline(rows: list) -> InlineKeyboardMarkup:
    ...
    for row in rows:
        ...
        kbd.append(raw.types.KeyboardButtonRow(buttons=buttons))
    return raw.types.ReplyInlineMarkup(rows=kbd)
