...
class AdminBot:
    ...
    async def set_bot_menu(self):
        try:
            # Publish every command that is actually handled by handle_command().
            # Keeping this list in sync prevents Telegram's menu from exposing
            # dead/removed commands such as the old mass_refer/stop_ref entries.
            commands = [
                BotCommand("start", "Open admin panel"),
                BotCommand("admin", "Open admin panel"),
                BotCommand("accounts", "Show accounts"),
                BotCommand("balances", "Show live balances"),
                BotCommand("stats", "Show performance stats"),
                BotCommand("guess", "Show adaptive forecast"),
                BotCommand("status", "Show RAM and uptime"),
                BotCommand("groups", "Show groups"),
                BotCommand("assignments", "Show assignments"),
                BotCommand("settings", "Show settings"),
                BotCommand("help", "Show all commands"),
                BotCommand("message", "Send a message from accounts"),
                BotCommand("claim", "Send claim command"),
                BotCommand("cancel", "Cancel active operation"),
                BotCommand("random", "Randomize schedules"),
                BotCommand("set_bal", "Set account balance"),
                BotCommand("set_balance_group", "Set balance group"),
                BotCommand("add_account", "Add account"),
                BotCommand("add_group", "Add group"),
                BotCommand("assign", "Assign account to group"),
                BotCommand("set_time", "Set target time"),
                BotCommand("unassign", "Remove assignment"),
                BotCommand("start_auto", "Start automation"),
                BotCommand("pause_auto", "Pause automation"),
                BotCommand("set_cycle", "Set cycle hours"),
                BotCommand("set_keywords", "Set trigger keywords"),
                BotCommand("set_action", "Set automation action"),
                BotCommand("set_slot", "Set slot schedule"),
                BotCommand("test_send", "Test group message"),
                BotCommand("limit", "Set extols limit"),
                BotCommand("shift", "Change admin view"),
                BotCommand("add_admin", "Add an admin"),
                BotCommand("del_admin", "Remove an admin"),
                BotCommand("delete_admin", "Remove an admin")
            ]
    ...
    def get_visible_accounts(self, chat_id: int) -> list[dict[str, Any]]:
        ...
        now = time.monotonic()
        if now - self._accounts_cache_at > 2.5:
    ...
    @staticmethod
    def _ram_usage() -> str:
        """Return container-aware RAM usage without depending on `free`.

        Railway/container environments can omit procps `free`, while cgroup
        memory files are normally available. Prefer the container limit so the
        percentage reflects the service's actual memory budget.
        """
        ...
        if current is not None and limit is not None and limit > 0 and limit < 1 << 60:
            pct = min(100.0, current / limit * 100.0)
            return f"{pct:.1f}% ({current // 1048576}MB / {limit // 1048576}MB)"
        ...
        # Host/proc fallback. This is still useful locally if no cgroup exists.
        try:
            info = {}
            ...
            if total and available is not None:
                ...
                return f"{used / total * 100:.1f}% ({used // 1048576}MB / {total // 1048576}MB)"
        except Exception:
            pass
        return "Unavailable"
    ...
    async def server_status(self, chat_id: int, message_id: int | None = None) -> None:
        ...
        text = (
            ...
            f"{E_USER} Your Accounts: <code>{len(self.get_visible_accounts(chat_id))}</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━"
        )
    ...
    async def reply(self, chat_id: int, text: str, markup: dict | None = None, message_id: int | None = None) -> None:
        try:
            if message_id:
                await self.app.edit_message_text(chat_id, message_id, text, reply_markup=markup, parse_mode=ParseMode.HTML)
            else:
                await self.app.send_message(chat_id, text, reply_markup=markup, parse_mode=ParseMode.HTML)
        except Exception as exc:
            err = str(exc)
            # Pressing the same button twice can legitimately produce this 400.
            # It is not a panel failure and must never be shown to the user.
            if "MESSAGE_NOT_MODIFIED" in err:
                return
            self.store.log("error", "Failed to reply", {"error": err})
