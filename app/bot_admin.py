...
━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    def _build_stats(self, chat_id: int) -> str:
        ...
        return (
            "📊 <b>Farm Analytics Report</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📅 Today's Profit: <code>{today_profit:,} Extols</code>\n"
            f"🗓 Last 7 Days: <code>{week_profit:,} Extols</code>\n"
    ...
    def _build_guess(self, chat_id: int) -> str:
        ...
        return (
            "🤖 <b>Smart ML Prediction Model</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚡️ Active Avg Speed: <code>{int(weighted_avg):,} Extols/day</code>\n"
            f"📈 Trend Momentum: <code>{trend_emoji}</code>\n\n"
    ...
    def _build_menu(self, chat_id: int) -> str:
        ...
        return ("⚙️ <b>SlotOps Admin Panel</b>\n━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    async def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        try:
            if data == "menu": 
                ...
                await self.reply(chat_id, "✅ Automation started.", main_keyboard(), message_id=message_id)
            elif data == "pause_auto":
                self.store.update_settings({"automation_enabled": False})
                await self.reply(chat_id, "❌ Automation paused.", main_keyboard(), message_id=message_id)
    ...
    async def shift_admin(self, chat_id: int, args: str) -> None:
        ...
        if args.lower() in ["reset", "me"]:
            ...
            CustomDB.set(f"shift_{chat_id}", args.strip()); await self.reply(chat_id, f"👤 Impersonating Admin: <code>{args}</code>", main_keyboard())
                    ...
                    if await client.is_user_authorized():
                        ...
                        for grp in assigned_groups:
                            try:
                                ...
                                success_count += 1
                                await self.reply(chat_id, f"✅ Claim sent from: {acc['label']} ➡️ {grp['title']}")
                            except Exception:
                                pass
    ...
    async def execute_mass_message(self, chat_id: int, target: str, message_text: str, count: int) -> None:
        try:
            ...
            if not my_accounts:
                await self.reply(chat_id, "❌ No accounts available.")
                return
            ...
            for i, acc in enumerate(selected_accounts):
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        ...
                        await client.send_message(actual_target, message_text)
                        success_count += 1
                        await self.reply(chat_id, f"✅ Message sent from: {acc['label']}")
                    await client.disconnect()
                except asyncio.CancelledError: raise
                except Exception as e: await self.reply(chat_id, f"❌ Failed from {acc['label']}: {str(e)}")
        ...
        try:
            if text.strip().isdigit() and len(text.strip()) <= 3:
                ...
                if pending_acc:
                    ...
                    await self.reply(chat_id, f"✅ Received Answer: <b>{text.strip()}</b>\nBot is pressing the button...", main_keyboard())
                    return
            ...
            if chat_id in MESSAGE_STATE:
                ...
                if state["step"] == "waiting_target":
                    ...
                    await self.reply(chat_id, f"✅ Target set: {state['target']}\n\n🔗 Step 2: What message do you want to send?")
                    return

                elif state["step"] == "waiting_message":
                    state["message"] = text.strip()
                    state["step"] = "waiting_count"
                    await self.reply(chat_id, f"✅ Message saved.\n\n👤 Step 3: How many accounts should send this message? (Enter a number or 'all')")
                    return
                    ...
                    if count_str == 'all':
                        ...
                        try:
                            ...
                            await self.reply(chat_id, "❌ Enter a valid number, 'all', or /cancel")
                            return
            ...
            if command in {"/start", "/admin"}: await self.send_menu(chat_id)
                ...
                await self.reply(chat_id, "🔗 Step 1: Send the target Username or Group Link (e.g., @username or https://t.me/...)")
                
            elif command == "/claim":
                if not args:
                    await self.reply(chat_id, "❌ Usage: <code>/claim MESSAGE</code>\nExample: <code>/claim /daily</code>", main_keyboard())
                    return
                
                my_accounts = self.get_visible_accounts(chat_id)
                if not my_accounts:
                    await self.reply(chat_id, "❌ No accounts available to claim.")
                    return
                ...
                await self.reply(chat_id, f"✅ Balance forwarding group set to: <b>{target}</b>", main_keyboard())
                ...
                if acc and grp:
                    ...
                    await self.reply(chat_id, "✅ Target Time set", main_keyboard())
                ...
                if acc and grp:
                    ...
                    await self.reply(chat_id, "❌ Removed.", main_keyboard())
