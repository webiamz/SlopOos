...
class AdminBot:
    ...
    async def reply(self, chat_id: int, text: str, markup: dict | None = None, message_id: int | None = None) -> None:
        try:
            ...
            # Fallback: agar markup/emoji ki wajah se fail hua ho, plain text (HTML-escaped) bhej ke user ko dikhao ki error kya tha
            safe_err = str(exc).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            try:
                await asyncio.to_thread(self._send_message_sync, chat_id, f"⚠️ Panel error: {safe_err}", None)
            except Exception:
                pass
    ...
    def _send_message_sync(self, chat_id: int, text: str, markup: dict | None = None) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"{exc.code}: {body}") from None
