...
class AdminBot:
    ...
    # --- THE STRICT ADMIN SEPARATION LOGIC ---
...
━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    async def execute_mass_refer(self, chat_id: int, args: str) -> None:
        try:
            ...
            channels = CustomDB.get(f"mass_refer_{chat_id}", [])
            if not channels:
                await self.reply(chat_id, "❌ Run <code>/mass_refer</code> first.")
                return
                
            # 🔥 Only use accounts visible to THIS admin
            my_accounts = self.get_visible_accounts(chat_id)
            if acc_target.lower() == "all": target_accounts = my_accounts
            else: target_accounts = [a for a in my_accounts if a["label"].lower() in [l.strip().lower() for l in acc_target.split(",")]]
                ...
                try:
                    ...
                    if await client.is_user_authorized():
                        for channel in channels:
                            ...
                            try:
                                if "joinchat/" in channel or "+" in channel: await client(ImportChatInviteRequest(channel.split("+")[-1] if "+" in channel else channel.split("joinchat/")[-1].strip("/")))
                        ...
                        if messages and messages[0].buttons:
                            ...
                            for row in msg.buttons:
                                for btn in row:
                                    if any(kw in btn.text.lower() for kw in ["claim", "joined", "check", "done", "verify"]):
                                        await btn.click(); clicked = True; await asyncio.sleep(2); break
                                if clicked: break
                            if not clicked:
                                try: await msg.click(len(msg.buttons) - 1, len(msg.buttons[-1]) - 1); await asyncio.sleep(2)
                                except Exception: pass

                        await asyncio.sleep(3) 
                        final_messages = await client.get_messages(target_bot, limit=1)
                        await self.reply(chat_id, f"📢 <b>Reply for</b> <code>{acc['label']}</code>:\n{final_messages[0].message if final_messages else 'No reply'}")
                        success_count += 1
                        await self.reply(chat_id, f"✅ <b>Account {i+1} complete:</b> <code>{acc['label']}</code>")
                        
                    await client.disconnect()
                except asyncio.CancelledError: raise
                except Exception as e: await self.reply(chat_id, f"❌ <b>Account {i+1} failed:</b> <code>{acc['label']}</code>")
                    
            if chat_id in self.active_tasks: del self.active_tasks[chat_id]
            await self.reply(chat_id, f"🎉 <b>Finished!</b>\nSuccess: {success_count}/{len(target_accounts)}")
        ...
        try:
            if chat_id in REFER_STATE:
                ...
                if state["step"] == "waiting_count":
                    try:
                        state["total"] = int(text.strip())
                        state["step"] = "waiting_links"
                        await self.reply(chat_id, f"✅ Send link 1 of {state['total']}:")
                    except ValueError: await self.reply(chat_id, "❌ Valid number please. Or /cancel.")
                    return
                elif state["step"] == "waiting_links":
                    if text == "/cancel": del REFER_STATE[chat_id]; await self.reply(chat_id, "🛑 Cancelled."); return
                    state["channels"].append(text.strip())
                    if len(state["channels"]) < state["total"]: await self.reply(chat_id, f"✅ Send link {len(state['channels']) + 1} of {state['total']}:")
                    else:
                        CustomDB.set(f"mass_refer_{chat_id}", state["channels"])
                        del REFER_STATE[chat_id]
                        await self.reply(chat_id, "🎉 <b>Channels set.</b>\nRun:\n<code>/ref all | @TargetBot | /start ref123</code>")
                    return

            if command in {"/start", "/admin"}: await self.send_menu(chat_id)
            elif command == "/mass_refer": REFER_STATE[chat_id] = {"step": "waiting_count", "channels": []}; await self.reply(chat_id, "❓ <b>How many channels to join?</b>")
            elif command == "/ref_link": self.active_tasks[chat_id] = asyncio.create_task(self.handle_ref_link(chat_id, args))
            elif command == "/cancel":
                if chat_id in REFER_STATE: del REFER_STATE[chat_id]
                await self.reply(chat_id, "🛑 Cancelled.", main_keyboard())
            elif command == "/ref": self.active_tasks[chat_id] = asyncio.create_task(self.execute_mass_refer(chat_id, args))
            elif command == "/stop_ref": 
                if chat_id in self.active_tasks: self.active_tasks[chat_id].cancel(); del self.active_tasks[chat_id]; await self.reply(chat_id, "🛑 <b>Task STOPPED!</b>")
                ...
                CustomDB.set(f"owner_{account['id']}", chat_id) # Save who owns this account
