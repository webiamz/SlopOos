    ...
    async def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        try:
            if data == "menu": 
                ...
                msg = "🛡️ Security System 🛡️\n\nNew logins are monitored interactively. If a new device connects, you will receive an alert to Approve or Kick the session."
                ...
                if len(parts) == 3:
                    ...
                    if account:
                        try:
                            ...
                            await self.reply(chat_id, f"✅ <b>Session Terminated!</b>\nIntruder removed from <code>{account['label']}</code>.", message_id=message_id)
                        except Exception as e:
                            await self.reply(chat_id, f"❌ Failed to kick: {e}", message_id=message_id)
